...
@Service
@Qualifier("peninsula")
public class NttDataManejoPedidosPeninsulaServiceImpl extends NttDataManejoPedidosServiceAbstract {

	@Override
	public void addOrder(Pedido order, Producto... products) {
		List<Producto> productList = Arrays.asList(products);
		productList.forEach(product	-> product.setPrecioPVP((product.getPrecio() * (1 + 0.21))));
